...
def divide_scalar(inputs, attrs):
    assert len(inputs) == 1
    return np.divide(inputs[0], attrs["scalar"])
...
def transpose(inputs, attrs):
    assert len(inputs) == 1
    axes = [i for i in range(inputs[0].ndim)]
    if attrs["axes"] is not None:
        axes[attrs["axes"][0]] = attrs["axes"][1]
        axes[attrs["axes"][1]] = attrs["axes"][0]
    else:
        if len(inputs[0].shape) < 2:
            ...
            axes[-1] = axes[-2]
            axes[-2] = temp
    return np.transpose(inputs[0], axes=axes)
...
def logsoftmax(inputs, attrs):
    assert len(inputs) == 1
    norm = inputs[0] - np.max(inputs[0], axis=1, keepdims=True)
    exp = np.exp(norm)
    expsum = np.sum(exp, axis=1, keepdims=True)
    return norm - np.log(expsum)
